...
# Tracks active users & dynamic active timeouts
active_users = {}
pending_private_requests = {}  # Format: { sender_sid: (target_sid, gevent_job) }
...
def broadcast_user_list_and_count():
    ...
    # Public counts broadcast updates restricted to main lobby
...
def handle_join(data):
    ...
    active_users[request.sid] = {
        ...
        "gender": gender,
        "status": "online",
        "room": "public"  # Default room allocation
    }
...
def handle_message(data):
    ...
    # Direct routing distribution using explicit user configuration channels
...
def auto_timeout_handler(sender_sid):
    gevent.sleep(10) # Strict 10-second request cycle restriction
...
def handle_private_response(data):
    target_sid = request.sid # The one who is responding
    ...
    timeout_job.kill() # Terminate countdown timer tracking execution thread
    ...
    # Generate dedicated encrypted pipeline space room ID allocation
    private_room_id = f"private_{sender_sid[:5]}_{target_sid[:5]}"
    
    # Detach both profiles securely from main network channels
    leave_room("public", sid=sender_sid)
    leave_room("public", sid=target_sid)
...
def handle_exit_private():
    ...
    # Locate tracking profile link associated with current matching set
    partner_sid = None
    ...
    # Process home lobby integration mechanics for current profile
    leave_room(private_room_id, sid=sid)
    join_room("public", sid=sid)
    active_users[sid]['room'] = "public"
    emit('returned_to_public_lobby')
    
    # Sync public lobby stream updates
    emit('system_message', {'msg': f"🟢 {user_info['username']} re-entered the public lobby"}, room="public", broadcast=True)
    
    # Notify and drop partner cleanly back into main array stream safely
    if partner_sid:
        leave_room(private_room_id, sid=partner_sid)
        join_room("public", sid=partner_sid)
        active_users[partner_sid]['room'] = "public"
        socketio.emit('returned_to_public_lobby', room=partner_sid)
        socketio.emit('system_message', {'msg': f"🟢 {active_users[partner_sid]['username']} re-entered the public lobby"}, room="public", broadcast=True)
...
def handle_disconnect():
    ...
    # Dynamic recovery cleanup if profile drops mid-transmission
    ...
    if user_info:
        if user_info['room'] != 'public':
            # Handle partner automated restoration pool if partner goes offline
            private_room_id = user_info['room']
            for s_id, info in active_users.items():
                if info['room'] == private_room_id:
                    leave_room(private_room_id, sid=s_id)
                    join_room("public", sid=s_id)
                    active_users[s_id]['room'] = "public"
                    socketio.emit('returned_to_public_lobby', room=s_id)
                    socketio.emit('system_message', {'msg': f"⚠️ Connection closed. Partner left the app."}, room=s_id)
        else:
